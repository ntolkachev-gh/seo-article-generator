...
class Article(Base):
    ...
    keywords = Column(JSONB, nullable=False)  # Временно оставляем как nullable=False для совместимости
    structure = Column(Text, nullable=False)  # Временно оставляем как nullable=False для совместимости
    article = Column(Text, nullable=False)  # Временно оставляем как nullable=False для совместимости
    seo_score = Column(Float, nullable=False)  # Временно оставляем как nullable=False для совместимости
    model_used = Column(String(50), nullable=False, default="unknown")
    # Временно комментируем новые поля до применения миграции
